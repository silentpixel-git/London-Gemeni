    ...
    <AnimatePresence>
      {isOpen && (
        ...
        >
          ...
          {/* Modal card */}
          <motion.div variants={zoomFadeVariants(reducedMotion)} className="relative w-full max-w-sm bg-lb-paper border border-lb-border rounded-xl shadow-2xl overflow-hidden">
            ...
            <div className="p-6 space-y-5">
              ...
              <div className="flex gap-2 pt-1">
                {!isFirstRun && (
                  <button
                    onClick={onClose}
                    className="flex-1 px-4 py-2.5 border border-lb-border text-lb-primary text-sm font-semibold rounded-lg hover:bg-lb-bg pressable"
                  >
                    Cancel
                  </button>
                )}
                <button
                  onClick={handleSave}
                  disabled={!canSave}
                  className={`flex items-center justify-center gap-2 px-4 py-2.5 bg-lb-primary text-lb-bg text-sm font-semibold rounded-lg hover:bg-lb-accent disabled:opacity-40 disabled:cursor-not-allowed pressable ${isFirstRun ? 'w-full' : 'flex-1'}`}
                >
