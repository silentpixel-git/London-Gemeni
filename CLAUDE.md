...
| `npm run qa:lambda-imports` | The one thing bundlers hide: `api/ai.ts` ships as a Vercel Node function transpiled **per file**, so native Node ESM resolves it and a relative import without a `.js` suffix throws `ERR_MODULE_NOT_FOUND` at module load, taking down `/api/ai` entirely. Walks the graph from `api/ai.ts` and fails unless every relative import is either fully type-only or `.js`-suffixed. Vite, tsx and `tsc` all resolve bare specifiers happily, so nothing else catches this | No |
